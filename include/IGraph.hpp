...
public:
  ...
  /// window. After all destroy the Circle \param pos the std::pair<T, T>
  ...
  virtual bool drawCircle(std::pair<T, T> pos, T radius, Color color) = 0;
  ...
  /// std::pair<T, T> position \param height for the height of the rectangle
  ...
  virtual bool drawText(std::pair<T, T> pos, const std::string &content) = 0;
  ...
  /// its own properties \param std::pair<T, T> pos to know where to display the
  /// entity in the window \return true if the entity is successfully draw
  virtual bool drawEntity(IEntity &entity, std::pair<T, T> pos) = 0;
